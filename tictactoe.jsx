const Board = () => {
  ...
  //let turn = '';
  // (player == 0) ? turn = 'Player O' : turn = 'Player X';

  console.log(`We have a winner ${status}`);

  const takeTurn = (id, symbol) => {
    setGameState([...gameState, { id: id, player: player }]);
    let updatePlayer = (player + 1) % 2;
    setTurn(symbol[updatePlayer]);
    setPlayer(updatePlayer); // get next player
    return player;
  };
